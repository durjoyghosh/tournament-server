...
const sport_model_1 = require("../modules/tournament/sport.model");
const tournament_model_1 = require("../modules/tournament/tournament.model");
const team_model_1 = require("../modules/team/team.model");
const player_model_1 = require("../modules/player/player.model");
const match_model_1 = require("../modules/match/match.model");
...
const seedDatabase = async () => {
    try {
        ...
        await sport_model_1.Sport.deleteMany({});
        await tournament_model_1.Tournament.deleteMany({});
        await team_model_1.Team.deleteMany({});
        await player_model_1.Player.deleteMany({});
        ...
        // Super Admins
        const superAdmin1 = await user_model_1.User.create({
            name: 'Durjoy Ghosh',
        ...
        for (let i = 0; i < 2; i++) {
            const org = await user_model_1.User.create({
        ...
        for (let i = 0; i < 10; i++) {
            const mgr = await user_model_1.User.create({
                name: managerNames[i],
        ...
        for (let i = 0; i < 10; i++) {
            const ref = await user_model_1.User.create({
                name: refereeNames[i],
        ...
        for (let i = 0; i < 10; i++) {
            const ply = await user_model_1.User.create({
                name: playerNames[i],
        ...
        for (const sData of sportsData) {
            const spt = await sport_model_1.Sport.create({
                ...sData,
                status: 'active',
                createdBy: superAdmin1._id,
            });
        ...
        const teams = [];
        for (let i = 0; i < 10; i++) {
            const tm = await team_model_1.Team.create({
                name: teamNames[i],
        ...
        for (let i = 0; i < 10; i++) {
            await player_model_1.Player.create({
        ...
        const formats = ['league', 'league', 'knockout', 'knockout', 'league', 'league', 'hybrid', 'knockout', 'hybrid', 'league'];
        const tournaments = [];
        for (let i = 0; i < 10; i++) {
            ...
            const tour = await tournament_model_1.Tournament.create({
        ...
        for (let i = 0; i < 10; i++) {
            ...
            await match_model_1.Match.create({
                tournament: tour._id,
